...
IF EXISTS (SELECT * FROM sysobjects WHERE name='M_AFD_FK' and xtype='F')
...
IF EXISTS (SELECT * FROM sysobjects WHERE name='M_CHEF_FK' and xtype='F')
	ALTER TABLE Medewerkers DROP CONSTRAINT M_CHEF_FK
GO
...
PRINT 'Tabel Medewerkers wordt aangemaakt'
GO
IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Medewerkers' and xtype='U')
	CREATE TABLE Medewerkers
	(
		Mnr INT CONSTRAINT M_PK PRIMARY KEY CONSTRAINT M_MNR_CHK CHECK(Mnr > 7000),
		...
		CONSTRAINT M_VERK_CHK CHECK((CASE Functie WHEN 'VERKOPER' THEN 0 ELSE 1 END + CASE WHEN Comm IS NULL THEN 0 ELSE 1 END) = 1)
	)
GO

PRINT 'Tabel Afdelingen wordt aangemaakt'
GO
IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='Afdelingen' and xtype='U')
	...
	(
		...
		Hoofd INT REFERENCES Medewerkers(Mnr) 
...
PRINT 'Tabel Schalen wordt aangemaakt'
GO
...
PRINT 'Tabel Cursussen wordt aangemaakt'
GO
...
PRINT 'Tabel Uitvoeringen wordt aangemaakt'
GO
...
PRINT 'Tabel Inschrijvingen wordt aangemaakt'
GO
...
PRINT ''
GO
PRINT 'Alle tabellen zijn aangemaakt. Script is correct uitgevoerd.'
GO